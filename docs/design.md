...
- **CPU Turn Opens Like a Human One**: the launch sits after `Match.StartTurn()` rather than after the human's `ResolveTurn()`, so sudden-death hazards are on the board before `Decide()` reads it — injection lives only in `StartTurn()`, and a CPU turn that skipped it would plan against a stale board. The client opens the CPU's turn with the same `POST /start-turn` it uses for its own, holding both player tokens and selecting by `ActiveTeam`; no token rule is relaxed and no response shape differs between the two paths.
- **Latency Masking**: CPU compute overlaps the sudden-death cutscene and turn banner, not the human's resolve animation — correct hazard ordering costs the larger masking window. A greedy planner projects to ~60ms even with future power-ups, skills and traps, so the wait stays imperceptible and a dedicated "AI is thinking" state is deferred. It becomes necessary if `Decide()` ever adopts adversarial lookahead, which is also the point where `runCPUTurn` must stop holding the room mutex for its full duration.
...
- **Bounded Replan on Divergence**: a failed apply triggers one re-`Decide()` call against the current `WorkingState` for the remaining scope, capped per turn; exhausting the cap aborts the remainder and resolves with whatever committed. Replanning is event-triggered on failure, never per-command.
- **Prologue Scope Bound**: a single `Archetype.Boss` unit with no ally coordination, combined with the 1-Move-1-Bomb action economy, bounds `Decide()`'s candidate space to five shapes (`nothing` / `move` / `bomb` / `move→bomb` / `bomb→move`) instead of requiring general multi-step search. The `[]TurnCommand` contract generalizes to multi-unit coordination later without an interface change.
