...
- The top **84px** is the `HeaderRegion`.
  - It contains a `BackButton`, then 48px of spacer, then the title of the current `Page`.
- The bottom **84px** is the `NavRegion`, which renders the Next/Start Match buttons.
...
In the body region, divide it horizontally into two `Panels`, `FormationPanel` and `ArchetypesPanel`.

In the `NavRegion`, render a **144Wx96Hpx** pill-shaped button (`NextButton`) — a `PillButton` whose size is overridden to `pill(144, 96)`, keeping the `PANEL_BUTTON_*` fill/border colors (so the size overrides `PANEL_BUTTON_WIDTH`/`HEIGHT` of 46×32). The button itself sits flush against the right edge of the `NavRegion`; its `NEXT →` label stays centered (`PillButton`'s default).

### Visual Effect of Formation Panel

`FormationPanel` covers **35%** of the top of `UnitPage`, which shows the team formation that the Player selects.
...
`ArchetypesPanel` covers the remaining **65%** of the `UnitPage`, which shows all `archetypes` obtained previously. Each `archetype` should convert to `Unit` represented by a `UnitCard`.

`ArchetypesPanel` lists all `UnitCards`. Each card should have **12px** spacing. At the moment there should be 4 `UnitCards` a row, each row horizontally centered within `ArchetypesPanel` (a row with fewer than 4 cards, e.g. the current 2 archetypes, centers just that row rather than sitting left-aligned in a full 4-column block). Note that in future there will be more than 16 archetypes available, so `ArchetypesPanel` should be horizontally swipable. Since there is not enough Archetype to scroll at the moment, the swipe effect is a **non-goal** unless it's very simple to do.

#### Unit Card

`UnitCard` is a **180Wx200Hpx** rounded rectangle container, with **12px** padding on 4 side.
...
In the `NavRegion`, render a **144Wx96Hpx** pill-shaped button (`StartMatchButton`) styled and positioned identically to `NextButton` (same `pill(144, 96)` size, `PANEL_BUTTON_*` colors, and flush against the right edge of the `NavRegion`). The button's `Start Match` label stays centered (`PillButton`'s default).
