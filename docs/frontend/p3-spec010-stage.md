...
> Note: Agent should correct me if `DevBootScene` is still necessary to bring `gameCfg` to `MatchSettingsScene`.

## StagePage

The main region of `StagePage` consists of 2 panels, `StagesPanel` and `StageDetailPanel`, sitting in 2 columns.
...
`StageCard` is a **180Wx200Hpx** rounded rectangle container, with **12px** padding on 4 side. [User Interaction](#user-interaction-and-visual-effect-of-stage-page) will describe a **Select** behavior. When a `StageCard` is selected, add a border of **4px** in HEX `0xdc9e23`.
...
Similar to `ArchetypesPanel` in `UnitPage`, `StagesDetailPanel` list all `stagePresets`. T

> Note: Agent should confirm if `InnerPanel` is necessary for this rendering.
...
- `{maxTurns}`: This is a selection component, with a `❰` and `❱` wrapped.
  - 2 arrows symbol should be at a fixed position, i.e. **24px** at the nearest edge. The font color is HEX `0xdc9e23`.

> Note: Agent should advise if `❰  {maxTurns}  ❱` on its own should deserve a component name.
...
- All `StageCards` in `StagesPanel` have click handlers.
  ...
  - `maxTurns` is cyclic selection component. The available selections are: `💣`, `15`, `20`, `30`, `45`, `60`. Note that `💣` means `0` in behind.
    - Unlike the other components that rely on `MatchSettingsScene.gameCfg`, `maxTurns` always render `stagePreset.maxTurns` first rather than the previous game data since it's the recommended max turns of that stage.
    - Add a click handler for `❰` - clicking this will shift the selection left, e.g., `20 -> 15`. Note that it's cyclic, i.e., `💣 -> 60`.
    - Add a click handler for `❱` - clicking this will shift the selection left, e.g., `15 -> 20`. Note that it's cyclic, i.e., `60 -> 💣`.
    - If the currently selected value is `stagePreset.maxTurns`, render a `🌟` with font size **8px** next to the value, meaning it's recommended max turns. Note that it should be a fix position - never move any elements to accommdate it.
